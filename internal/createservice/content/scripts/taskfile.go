...
tasks:
  ...
  grpc_generate:
    ...
    dir: ../
    cmds:
      - protoc --proto_path=api/protobuf/protofiles ./api/protobuf/protofiles/<service-name>/<service-name>.proto --go_out=./api/protobuf/generated/go --go_opt=paths=source_relative --go-grpc_out=./api/protobuf/generated/go --go-grpc_opt=paths=source_relative --experimental_allow_proto3_optional
  ...
  migrate:
    ...
    vars:
      ...
      DATABASE_URL: "postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@$HMTM_<service-name-upper>_DB_OUTER_HOST:$HMTM_<service-name-upper>_DB_OUTER_PORT/$POSTGRES_DB"
    dir: ../
    cmds:
      - go install github.com/pressly/goose/v3/cmd/goose@latest
  ...
  upload_docker_image:
    ...
    cmds:
      - task: prepare_env_file_prod
      - task: docker_build_prod
      - sudo docker tag hmtm_<service-name> d3m0s/hmtm_<service-name>
      - sudo docker push d3m0s/hmtm_<service-name>
`
